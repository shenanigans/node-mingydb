...
Collection.prototype.geoNear = function (x, y, options, callback) {
    ...
    if (options.query)
        return this.compressor.compressQuery (query, function (err, compressedQuery) {
            if (err) return callback (err);
            if (!geoPoint) {
                // legacy point
...
*/
Collection.prototype.geoHaystack = function (x, y, options, callback) {
    var geoPoint;
    if (arguments.length == 2) {
        // geoJSON
        geoPoint = x;
    ...
    var self = this;
    function finalCall (err, recs) {
        if (err) return callback (err);
        if (!recs || !recs.length)
            return callback (undefined, recs);
        var output = [];
        async.times (recs.length, function (recI, callback) {
            self.compressor.decompress (recs[recI], function (err, decompressed) {
                if (err) return callback (err);
    ...
    if (options.query)
        return this.compressor.compressQuery (query, function (err, compressedQuery) {
            ...
            if (!geoPoint)
                return self.collection.geoHaystack (x, y, newOptions, finalCall);
            // manual command
            newOptions.geoNear = self.collectionName;
            newOptions.near = geoPoint;
            self.collection.db.command (newOptions, finalCall);
        });
...
/**     @member/Function Collection#setUncompressed
