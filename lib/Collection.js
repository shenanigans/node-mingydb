...
function Collection (colName, collection, compressor) {
    this.name = colName;
...
/**     @member/Function Collection#insert
    Compress and insert a record into the database. Unlike native insertion, the callback is not
    optional. This is because the record [compression](waxwork.this.compressor) stage may generate an
    Error.
@argument/Object rec
@callback
    @argument/Error|undefined err
*/
Collection.prototype.insert = function (rec, options, callback) {
    ...
    if (this.fatalError) {
        if (callback) process.nextTick (function(){ callback (self.fatalError); });
        return;
    }

    this.compressor.compressRecord (rec, function (err, compressed) {
        if (err) {
            if (callback) callback (err);
            return;
        }
        self.collection.insert (compressed, options, function (err) {
            callback (err);
        });
    });
};
...
/**     @member/Function Collection#find
    Compress a query and pass it to the database, producing a [cursor](waxwork.Cursor).
...
@callback
    @argument/Error|undefined err
    @argument/waxwork.Cursor|undefined cursor
*/
...
*/
Collection.prototype.aggregate = function (callback) {
