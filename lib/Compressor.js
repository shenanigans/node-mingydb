    ...
    for (var i in pathFrags) {
        ...
        return async.eachSeries (rkeys, function (stepI, callback) {
            ...
            self.minsCollection.findOne ({ p:parent._id, l:step }, function (err, rec) {
                if (err) return callback (err);
                if (!rec) return callback (new Error ('cannot resolve path '+fullpath));
...
Compressor.prototype.decompressArray = function (arr, shortpath, longpath, parent, callback) {
    ...
    async.each (indexes, function (i, callback) {
        ...
        var type = getTypeStr (val);
        if (type == 'array')
            return self.decompressArray (val, shortpath, longpath, parent, function (err, decompressedElem) {
                if (err) return callback (err);
                newArray[i] = decompressedElem;
                callback();
            });
        if (type == 'object')
            return self.decompressLevel (val, shortpath, longpath, parent, function (err, decompressedElem) {
                if (err) return callback (err);
                newArray[i] = decompressedElem;
                callback();
            });
    ...
    async.times (pipe.length, function (stageI, callback) {
        ...
        if (operator == '$geoNear') {
            ...
            return async.parallel (jobs, function (err) {
                ...
                compressedPipe[stageI] = newExpression;
                callback();
            });
        }
        ...
        if (operator == '$group') {
            ...
            var compressedSpec = {};
            return async.each (Object.keys (paths), function (pathsI, callback) {
                var path = paths[pathsI];
                var compressedPath, compressedExpression;
                async.parallel ([
                    function (callback) {
                        self.compressAndEnsurePath (
                            self.documentClass,
                    ...
                    function (callback) {
                        self.compressExpression (path, function (err, compressed) {
                            if (err) return callback (err);
                ...
                callback (err, err ? undefined : compressedSpec);
            });
        }
        ...
        if (operator == '$projection') {
            ...
            return async.each (Object.keys (expression), function (key, callback) {
                ...
                callback (err, err ? undefined : newSpec);
            });
        }
...
Compressor.prototype.compressExpression = function (expression, callback, root) {
    ...
    if (type == 'object') {
        ...
        return this.compressExpression (expression, callback, root);
    }
    ...
    if (type == 'array') {
        ...
        return async.times (expression.length, function (i, callback) {
            self.compressExpression (newExpression[i], function (err, compressedExpression) {
                if (err) return callback (err);
