...
*/
Compressor.prototype.compressAggregationPipeline = function (pipeline, callback, staticNamespace) {
    var self = this;
    var compressedPipe = [];
    var crossover;
    ...
    if (lastStageKeys[0] == '$out' && lastStage.$out != this.documentClass._id) {
        if (!secondNamespace)
            secondNamespace = { _id:lastStage.$out };
        ...
        if (crossover === undefined)
            if (secondNamespace.A != this.documentClass._id)
            ...
            else // the second namespace is an alias to this one
                crossover = pipeline.length; // no stage can hold this index
    } else
        crossover = pipeline.length; // no stage can hold this index


    async.times (pipeline.length, function (stageI, callback) {
        var pathNamespace = stageI < crossover ? self.documentClass : secondNamespace;
        var expressionNamespace = stageI <= crossover ? self.documentClass : secondNamespace;
...
Compressor.prototype.compressExpression = function (expression, callback, root) {
    ...
    if (type == 'object') {
        ...
        if (firstKey == '$cond') {
            var rootCheck = JSON.stringify (root);
            var newExpression = {};
            return async.parallel ([
                function (callback) {
